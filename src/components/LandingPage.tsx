    ...
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <header className="absolute top-0 left-0 right-0 z-10 px-6 py-4 flex items-center justify-between max-w-5xl mx-auto w-full">
        <div className="flex items-center gap-2.5">
          <CasePlanrLogo className="w-12 h-12 text-accent-600" />
          <span className="text-2xl font-black text-slate-900 tracking-tight">CasePlanr</span>
        </div>
        <a
          href="/cases"
          className="px-4 py-2 bg-accent-600 hover:bg-accent-700 text-white text-sm font-bold rounded-xl transition-colors"
        >
          Sign In
        </a>
      </header>
      ...
      <section className="relative overflow-hidden bg-[radial-gradient(ellipse_100%_70%_at_50%_100%,#dbeafe,#ffffff)]">
        <div className="relative max-w-5xl mx-auto px-6 pt-28 pb-20 md:pt-32 md:pb-24 text-center">
          <h1 className="text-5xl md:text-6xl font-black text-slate-900 tracking-tight leading-[1.05] mb-5">
            Case plans that<br />
            <span className="text-accent-600">keep cases moving.</span>
          </h1>
        ...
        <section className="max-w-5xl mx-auto px-6 py-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURES.map(({ icon: Icon, title, description }) => (
              <div key={title} className="bg-white border border-slate-100 rounded-2xl p-6 space-y-3 shadow-sm hover:shadow-md hover:border-accent-100 transition-all">
                <div className="w-9 h-9 bg-accent-50 rounded-xl flex items-center justify-center">
                  <Icon className="w-4.5 h-4.5 text-accent-600" strokeWidth={2} />
                </div>
      ...
      <footer className="bg-slate-50 border-t border-slate-100 py-6 px-6">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
          ...
          <div className="flex items-center gap-4">
            <a href="/privacy" className="hover:text-accent-600 transition-colors">Privacy Policy</a>
            <span className="text-slate-300 select-none">&middot;</span>
            <a href="/terms" className="hover:text-accent-600 transition-colors">Terms of Service</a>
          </div>
        </div>
      </footer>
